...
namespace CityFlow {
    ...
    bool Engine::loadConfig(const std::string &configFile) {
        ...
        if (!readJsonFromFile(configFile, document) && !readJsonFromString(configFile, document)) {
...
//            return false;
//        }
//        if (!readJsonFromString(configFile, document)) {
//            std::cerr << "cannot open config file!" << std::endl;
//            return false;
//        }
            ...
            while (vehicleItr != vehicles.end()) {
                ...
                if (vehicle->hasSetEnd()) {
                    ...
                    if (!vehicle->getLaneChange()->hasFinished()) {
                        ...
                        cumulativeTravelTimeSquare += (getCurrentTime() - vehicle->getEnterTime()) * (getCurrentTime() - vehicle->getEnterTime());
                        vehicleTravelTime.push_back(getCurrentTime() - vehicle->getEnterTime());
                    }
...
//                    assert(vehicle->getPartner() == nullptr);
    ...
    std::list<double> Engine::getVehicleTravelTime() const {
        std::list<double> ret = vehicleTravelTime;
        for (auto &vehicle_pair : vehiclePool) {
            auto &vehicle = vehicle_pair.second.first;
            ret.push_back(getCurrentTime() - vehicle->getEnterTime());
        }
        return ret;
    }
    ...
    double Engine::getStdTravelTime() const {
        double tt = cumulativeTravelTime;
        double tt_square = cumulativeTravelTimeSquare;
        int n = finishedVehicleCnt;
        for (auto &vehicle_pair : vehiclePool) {
            auto &vehicle = vehicle_pair.second.first;
            double passed_time = getCurrentTime() - vehicle->getEnterTime();
            tt += passed_time;
            tt_square += passed_time * passed_time;
            n++;
        }
        return n <= 1? 0 : sqrt(tt_square / n - (tt / n) * (tt / n));
    }

    int Engine::getFinishedVehicleCnt() const {
    ...
    void Engine::reset(bool resetRnd) {
        ...
        cumulativeTravelTimeSquare = 0;
        vehicleTravelTime.clear();
